...
through this engine with a REAL broker (DemoBroker), which itself refuses unless
the demo env gate is green (TRADING_MODE=DEMO + creds + testnet endpoint).

Engine gates before ANY order (task §8):
  1. eligibility gate (execution.eligibility) — frozen source, approved risk
  2. spot constraint: LONG only (SHORT rejected — not executable on spot)
...
passes execution.demo_broker.DemoBroker. Nothing in this module ever imports
...
class DemoEngine:
    def __init__(self, broker, fee_bps: float | None = None, max_hold_bars: int = MAX_HOLD_BARS):
        """broker: object with market_buy/market_sell/order_status/validate (Fake or real)."""
        self.broker = broker
        s = get_settings()
    ...
    def open_from_candidate(self, candidate: dict) -> dict:
        ...
        # 2. spot constraint
        if side != "LONG":
            return self._reject(cand, "SPOT_LONG_ONLY", "spot demo supports LONG only")
        if str(cand.get("environment") or "DEMO").upper() != "DEMO":
        ...
        order_id = self._persist_order(cand, qty, "NEW", reject_reason=None)
        try:
            resp = self.broker.market_buy(symbol, qty)
        except Exception as e:
        ...
        if status in ("FILLED", "PARTIALLY_FILLED") and filled_qty > 0:
            ...
            pos = self._open_position(cand, order_id, entry_px, filled_qty)
            ev = {"symbol": symbol, "side": side, "entry": entry_px, "stop": cand.get("stop"),
                  ...
                  "timeframe": cand.get("timeframe"), "environment": "DEMO",
                  "decision_id": cand.get("decision_id"),
                  "id": cand.get("decision_id")}
    ...
    def _close_position(self, p: dict, hit: str, level_px: float, bar_ts: int) -> dict:
        ...
        # sell back to the broker (mock ok / real spot sell)
        sell = None
        try:
            sell = self.broker.market_sell(sym, qty)
        except Exception:
            sell = None  # exit bookkeeping proceeds; reconcile later if needed
        ...
        trade = store.insert_demo_trade({
            ...
            "opened_at": p["opened_at"], "closed_at": bar_ts, "environment": "DEMO"})
        rec = {**p, "exit_reason": hit, "exit_price": exit_px, "pnl": net, "fees": fees,
               "closed_at": bar_ts, "trade_id": trade.get("id"), "sell": sell,
               "qty": qty, "entry": entry}
    ...
    def _persist_order(self, cand: dict, qty: float, status: str,
        ...
        o = store.insert_demo_order({
            ...
            "ai_status": cand.get("ai_status"), "environment": "DEMO",
    ...
    def _open_position(self, cand: dict, order_id: str, entry_px: float,
        ...
        pos = store.insert_demo_position({
            ...
            "opened_at": int(time.time() * 1000), "environment": "DEMO"})
        return pos
